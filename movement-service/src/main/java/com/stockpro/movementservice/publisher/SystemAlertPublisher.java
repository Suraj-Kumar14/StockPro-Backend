...
public class SystemAlertPublisher {
    ...
    public void publish(String routingKey, SystemAlertEvent event) {
        try {
            ...
            log.info("Published movement-service system alert routingKey={} title={}", routingKey, event.title());
        } catch (Exception ex) {
            log.error("Failed to publish movement-service system alert routingKey={} error={}", routingKey, ex.getMessage(), ex);
        }
    }
}
