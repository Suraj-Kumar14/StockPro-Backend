...
public class SystemAlertPublisher {
    ...
    public void publish(String routingKey, SystemAlertEvent event) {
        try {
            ...
            log.info("Published purchase-service system alert routingKey={} title={}", routingKey, event.title());
        } catch (Exception ex) {
            log.error("Failed to publish purchase-service system alert routingKey={} error={}", routingKey, ex.getMessage(), ex);
        }
    }
}
